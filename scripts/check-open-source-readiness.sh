...
required_files=(
  ...
  .agents/skills/cut-artisan-release/SKILL.md
)
...
rg --fixed-strings --quiet "Build a native macOS quick-edit code editor" "$ROOT_DIR/SPEC.md" || fail "SPEC must describe the product"
rg --fixed-strings --quiet "External contributions are not currently supported" "$ROOT_DIR/CONTRIBUTING.md" || fail "CONTRIBUTING must state the contribution policy"
rg --fixed-strings --quiet "SPEC.md" "$ROOT_DIR/CONTRIBUTING.md" || fail "CONTRIBUTING must point forks to SPEC.md"
